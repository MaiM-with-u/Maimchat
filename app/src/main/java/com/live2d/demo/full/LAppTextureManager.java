...
public class LAppTextureManager {
    ...
    public TextureInfo createTextureFromBitmap(Bitmap bitmap, String key) {
        if (bitmap == null) {
            return null;
        }
        deleteTexture(key);
    ...
    public void deleteTexture(String key) {
        Iterator<TextureInfo> iterator = textures.iterator();
        while (iterator.hasNext()) {
            TextureInfo textureInfo = iterator.next();
            if (textureInfo.filePath.equals(key)) {
                int[] ids = new int[] { textureInfo.id };
