...
public class LAppLive2DManager {
    ...
    public void setUpModel() {
        ...
        Context ctx = LAppDelegate.getInstance().getContext();
        if (ctx == null) {
            throw new IllegalStateException("Context not initialized for Live2D.");
        }
    ...
    public void onUpdate() {
        ...
        float[] storedMatrix = Live2DViewTransformStore.INSTANCE.getMatrixCopy(activeTransformKey);
        if (storedMatrix != null) {
            viewMatrix.setMatrix(storedMatrix);
            Log.v("L2DMatrixApply", "key=" + activeTransformKey + " scaleX=" + storedMatrix[0] + " scaleY="
                    + storedMatrix[5] + " transX=" + storedMatrix[12] + " transY=" + storedMatrix[13]);
        } else {
            viewMatrix.loadIdentity();
            Log.v("L2DMatrixApply", "key=" + activeTransformKey + " -> identity fallback");
        }
    ...
    /**
     * Load specific model folder by name without relying on index.
     * 
     * @param folderName folder under assets root.
     * @return true if model loaded.
     */
    public boolean loadModelByFolder(String folderName) {
        if (folderName == null || folderName.isEmpty()) {
            return false;
        }
        try {
            Log.d("LAppLive2DManager",
                    "loadModelByFolder start folder=" + folderName + " thread=" + Thread.currentThread().getName());
            Context ctx = LAppDelegate.getInstance().getContext();
            if (ctx == null) {
                Log.e("LAppLive2DManager", "Context not initialized when loading folder=" + folderName);
                throw new IllegalStateException("Context not initialized for Live2D.");
            }
            if (!modelDir.contains(folderName)) {
                Log.d("LAppLive2DManager", "Folder " + folderName + " not in cache, refreshing modelDir");
                setUpModel();
            }
            int index = modelDir.indexOf(folderName);
            if (index < 0) {
                Log.w("LAppLive2DManager", "Folder " + folderName + " still missing after refresh, adding manually");
            ...
            releaseAllModel();
            Log.d("LAppLive2DManager", "Existing models released, creating new model for " + folderName);

            models.add(new LAppModel());
            models.get(0).loadAssets(modelPath, modelJsonName);
            Log.d("LAppLive2DManager", "Primary model assets loaded for " + folderName);
            ...
            LAppDelegate.getInstance().getView().switchRenderingTarget(useRenderingTarget);
            float[] clearColor = { 0.0f, 0.0f, 0.0f };
            LAppDelegate.getInstance().getView().setRenderingTargetClearColor(clearColor[0], clearColor[1],
                    clearColor[2]);

            currentModel = index >= 0 ? index : 0;
            Log.d("LAppLive2DManager",
                    "loadModelByFolder success folder=" + folderName + " modelCount=" + models.size());
            return true;
        } catch (Throwable t) {
            Log.e("LAppLive2DManager", "Failed to load model folder " + folderName, t);
            return false;
        }
    }
     ...
     *
     * @return number fo models in this LAppLive2DManager instance has. If models
    ...
    /**
     * Return a copy of current model directory list.
     */
    ...
    /**
     * 更新当前视图矩阵，用于同步双指缩放/拖动效果。
     *
     * @param matrix 最新的视图矩阵
     */
    public void updateViewMatrix(CubismViewMatrix matrix) {
        if (matrix == null) {
            return;
        }
        float[] arr = matrix.getArray();
        Log.v("L2DMatrixApply", "update key=" + activeTransformKey + " scaleX=" + arr[0] + " scaleY=" + arr[5]
                + " transX=" + arr[12] + " transY=" + arr[13]);
        Live2DViewTransformStore.INSTANCE.saveFrom(activeTransformKey, matrix);
        viewMatrix.setMatrix(matrix.getArray());
    }

    public void setActiveTransformKey(String key) {
        if (key == null || key.isEmpty()) {
            activeTransformKey = "default";
        } else {
            activeTransformKey = key;
        }
    }
    ...
    private String activeTransformKey = "default";
}
