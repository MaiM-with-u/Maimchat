...
public class LAppModel extends CubismUserModel {
    ...
    public void update() {
        // 获取原始时间增量
        final float originalDeltaTimeSeconds = LAppPal.getDeltaTime();

        // 应用倍速控制 - 通过Kotlin的倍速控制器获取修正后的时间增量
        float deltaTimeSeconds = originalDeltaTimeSeconds; // 默认使用原始时间增量
        try {
            // 使用反射调用Kotlin的倍速控制器
            Class<?> speedControlClass = Class.forName("com.example.myapplication.Live2DMotionSpeedControl");
            java.lang.reflect.Method getModifiedDeltaTimeMethod = speedControlClass.getMethod("getModifiedDeltaTime");
            Object speedControlInstance = speedControlClass.getField("INSTANCE").get(null);
            deltaTimeSeconds = (Float) getModifiedDeltaTimeMethod.invoke(speedControlInstance);
        } catch (Exception e) {
            // 如果倍速控制器不可用，使用原始时间增量（已在上面设置）
            // deltaTimeSeconds = originalDeltaTimeSeconds; // 不需要重新赋值
        }
        ...
        // ドラッグによる目の向きの調整
        model.addParameterValue(idParamEyeBallX, dragX); // -1から1的值を加える
        model.addParameterValue(idParamEyeBallY, dragY);

        // Breath Function - 使用修正后的时间增量
        if (breath != null) {
            breath.updateParameters(model, deltaTimeSeconds);
        }

        // Physics Setting - 使用修正后的时间增量
        if (physics != null) {
    ...
    public int startMotion(final String group,
        ...
        return startMotion(group, number, priority, /* loop */false, /* loopFadeIn */false, onFinishedMotionHandler,
                onBeganMotionHandler);
    }

    /**
     * 引数で指定したモーションの再生を開始する（ループ指定対応）。
     *
     * @param group                   モーショングループ名
     * @param number                  グループ内の番号
     * @param priority                優先度
     * @param loop                    ループ再生するか
     * @param loopFadeIn              ループ時にフェードインを行うか
     * @param onFinishedMotionHandler モーション再生終了時に呼び出されるコールバック関数。nullの場合は呼び出されない。
     * @param onBeganMotionHandler    モーション再生開始時に呼び出されるコールバック関数。nullの場合は呼び出されない。
     * @return 開始したモーションの識別番号。開始できない時は「-1」
     */
    public int startMotion(final String group,
            int number,
            int priority,
        ...
        if (motion != null) {
            motion.setLoop(loop);
            motion.setLoopFadeIn(loopFadeIn);
        }
        ...
        if (debugMode) {
            LAppPal.printLog("start motion: " + group + "_" + number + (loop ? " (loop)" : ""));
        }
     ...
     * @param group    モーショングループ名
     * @param priority 優先度
     * @return 開始したモーションの識別番号。個別のモーションが終了したか否かを判別するisFinished()の引数で使用する。開始できない時は「-1」
     */
    ...
    public void draw(CubismMatrix44 matrix) {
        ...
        // キ��ッシュ変数の定義を避けるために、multiplyByMatrix()ではなく、multiply()を使用する。
        ...
        if (renderer == null) {
            CubismRenderer recreated = CubismRendererAndroid.create();
            if (recreated != null) {
                setupRenderer(recreated);
                renderer = this.<CubismRendererAndroid>getRenderer();
                LAppPal.printLog("Renderer for model was null at draw time, recreated instance.");
            }
        }

        if (renderer == null) {
            LAppPal.printLog("Renderer unavailable, skip draw to prevent crash.");
            return;
        }
        ...
        try {
            ...
            boolean recoverable = drawError instanceof IndexOutOfBoundsException;
            if (!recoverable && drawError instanceof NullPointerException) {
                StackTraceElement[] trace = drawError.getStackTrace();
                if (trace != null) {
                    for (StackTraceElement element : trace) {
                        if (element.getClassName().contains("CubismRendererAndroid") ||
                                element.getClassName().contains("CubismShaderAndroid")) {
                            recoverable = true;
                            break;
                        }
                    }
            ...
            if (recoverable) {
                LAppPal.printLog("Renderer drawModel failed (" + drawError.getClass().getSimpleName()
                        + "), attempting renderer recreation.");
                CubismRenderer recreated = CubismRendererAndroid.create();
                if (recreated != null) {
                    setupRenderer(recreated);
                    CubismRendererAndroid retryRenderer = this.<CubismRendererAndroid>getRenderer();
                    if (retryRenderer != null) {
                        retryRenderer.setMvpMatrix(matrix);
                        try {
                            retryRenderer.drawModel();
                            LAppPal.printLog("Renderer drawModel recovered after recreation.");
                            return;
                        } catch (RuntimeException retryError) {
                            LAppPal.printLog("Renderer drawModel retry failed: " + retryError);
                            throw retryError;
                        }
                    }
                }
